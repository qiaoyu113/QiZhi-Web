<template>
  <div class="payresult">
    <div v-if="result == 1" class="pay payok">
        <i class="iconfont icon-icon_right"></i>
        <p class="title">付款成功</p>
        <p class="des">恭喜您订阅成功，可在 <router-link to="">个人中心-我的订阅</router-link> 中查看。选择合同模板开始创建吧</p>
        <div @click="finished()">完成</div>
    </div>
    <div v-if="result == 0" class="pay payno">
...
<script>
    export default {
      data(){
        return{
            result:this.$route.params.state
        }
      },
        props: [''],
        updated() {
