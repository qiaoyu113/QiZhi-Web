...
-->
<template>
    <router-link :to="{name:'author',params:{id:item.adminId}}" class="recommendauth_list">
        <div class="recommendauth_list_left float-left">
            <img :src="item.poster| picTurn">
        </div>
        <div class="recommendauth_list_center float-left">
            <p class="title">{{item.title}}</p>
            <div class="detail">{{item.summary}}</div>
        </div>
        <div class="recommendauth_list_right float-right">
            <div class="care" @click="postFollow(item.adminId)">
                <i></i>
                <span>关注</span>
            </div>
        </div>
...
<style>
/*作者推荐*/
.recommendauth_list{background:#fafafa;width:360px;height:110px;position: relative}
...
.recommendauth_list_more .see-more{display:inline-block;background:url("../../assets/image/see-more.png");border-radius:46px;width:280px;height:34px;line-height: 34px;font-size:14px;color:#999999;text-align:center;margin: 0 auto;cursor: pointer}
