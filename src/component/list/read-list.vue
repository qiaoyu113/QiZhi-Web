...
-->
<template>
    <div class="sy-list sy dicContent" id="read">
        <div class="bg_list">
            ...
            <div class="bg_list_right">
                <div class="devheight">
                    ...
                    <div class="bg_list_right_detail">
                        <router-link :to="{name:'passage',params:{id:item.id}}" v-html="item.desc"></router-link>
                    </div>
                </div>
                <div class="bg_list_right_bottom">
                    <div class="float-left indust_label" >
                        <a>共{{item.singleNum}}期</a>
                    </div>
                    <div class="author_info float-right">
                        <span class="price" v-if="item.price != 0">￥{{item.price/100}}</span>
...
<style lang="less">
    #read{
        ...
        .bg_list_right_detail{margin:10px 0 22px 0;font-size:12px;color:#84898f;text-align:left;margin-top:15px;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; overflow: hidden;}
